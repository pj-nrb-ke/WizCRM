...
| Item | Location |
|------|----------|
| Credentials (your file) | **`docs/brevo.local.txt`** (gitignored) — also `config/secrets/brevo.local.txt` |
| Loader | `api/src/services/brevo-config.ts` |
| Sender | `api/src/services/brevo-mail.ts` (Brevo API → SMTP fallback) |
| API | `GET /email/status`, `POST /email/leads/:leadId/send` |
| Mobile | Lead detail → email draft → **Send email (Brevo)** |
| Validate | `npm run email:validate -w api` |
| Test send | `npm run email:test -w api -- you@example.com` |

Health: `GET /health` includes `emailConfigured` and `emailMethod`.

After deploy, copy `docs/brevo.local.txt` to the VPS at `/opt/wizcrm/docs/brevo.local.txt` and restart `wizcrm-api`.

---

This document also describes how WizFlow integrates transactional email via [Brevo](https://app.brevo.com), using a **gitignored secrets file** that other agents and apps can reuse without committing credentials.
...
Optional fallbacks (also gitignored):

- `docs/brevo.local.txt` — **WizCRM default location**
