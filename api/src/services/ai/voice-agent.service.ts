 ...
 *
 * Returns why it failed as well as what it heard. An empty transcript is a
 * legitimate outcome (the caller stayed silent) but it is indistinguishable
 * from a broken download unless we say which happened — and a silent failure on
 * a phone call is the hardest kind to chase.
 */
export type TranscribeResult = {
  text: string;
  reason: 'ok' | 'no_client' | 'fetch_failed' | 'http_error' | 'too_small' | 'whisper_failed' | 'empty_transcript';
  status?: number;
  bytes?: number;
  contentType?: string;
  detail?: string;
...
export async function transcribeRecordingUrl(url: string): Promise<TranscribeResult> {
  ...
  try {
    const text = await transcribeAudio(client, buf.toString('base64'), extensionFor(url));
    const trimmed = text.trim();
    return {
      text: trimmed,
      reason: trimmed ? 'ok' : 'empty_transcript',
