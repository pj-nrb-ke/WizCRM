...
# Create api/.github-token.local (gitignored) with one line: ghp_xxxx or github_pat_xxxx
# Scopes: classic "admin:public_key" OR fine-grained repo access + account permission for SSH keys.
$ErrorActionPreference = 'Stop'
$Root = Split-Path $PSScriptRoot -Parent
$TokenFile = Join-Path $Root 'api\.github-token.local'
...
if (-not (Test-Path $TokenFile)) {
  Write-Host 'Missing api\.github-token.local — add a GitHub PAT (one line) to auto-register the SSH key.' -ForegroundColor Yellow
  exit 1
}
